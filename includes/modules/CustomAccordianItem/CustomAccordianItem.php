...
class Wpd_Accordian_Item extends ET_Builder_Module {
	...
	public function render( $attrs, $render_slug, $content = null ) {
		...
		// $accordion_description = $multi_view->render_element(
		// 	array(
		// 		'tag'      => 'div',
		// 		'content'  => '{{accordian_content}}',
		// 		'attrs'    => array(
		// 			'class' => 'wpd-accordion-description',
		// 		),
		// 		'required' => 'accordian_content',
		// 	)
		// );
		...
		$output = sprintf(
			'<div class="wpd_accordian_contents">
				<h3 class="wpd-accordion-title">
					<span class="et-pb-icon et-pb-font-icon">%4$s</span>%1$s
				</h3>
			...
			$accordion_content,
			$accordion_image,
			!empty($accordion_icon) ? sprintf('<span class="et-pb-icon et-pb-font-icon">%s</span>', $accordion_icon) : '',
			$read_more,
		);
